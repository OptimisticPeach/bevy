use crate::{
    ...
    bundle::Bundle,
    component::{Component, ComponentDescriptor, ComponentTicks, DataKindId, StorageType},
    entity::Entity,
...
unsafe impl<T: Component> FetchState for WithState<T> {
    ...
    #[inline]
    fn update_component_access(&self, access: &mut FilteredAccess<DataKindId>) {
...
unsafe impl<T: Component> FetchState for WithoutRelationState<T> {
    ...
    fn update_component_access(&self, _access: &mut FilteredAccess<DataKindId>) {
...
impl<T: Component> WorldQuery for With<Relation<T>> {
    type Fetch = WithRelationFetch<T>;
...
pub struct WithRelationFetch<T> {
    storage_type: StorageType,
    marker: PhantomData<T>,
}

impl<'w, 's, T: Component> Fetch<'w, 's> for WithRelationFetch<T> {
    type Item = bool;
    type State = WithRelationState<T>;
    type RelationFilter = SmallVec<[Entity; 4]>;
...
pub struct WithBundleState<T: Bundle> {
    relation_kind_ids: Vec<(DataKindId, Option<Entity>)>,
    is_dense: bool,
    marker: PhantomData<T>,
}
